...
SYSMINVAL = np.finfo(np.float64).tiny
...
class SequenceLoader:
    ...
    def translate(clc, core_inst, gcode=1, codemap={}):
        ...
        if isinstance(core_inst, six.string_types):
            ...
            for s in seqs:
                ...
                if len(s) % 3 != 0:
                    ...
                    aa_list = [seq_code.forward_table.get(
                        s.seq[i:i + 3].upper(), stop_checker((s.seq[i:i + 3], seq_code))) for i in range(0, len(s), 3)]
...
class SequenceSet(object):
    ...
    def fusion(self, *seqsetlist, **kwargs):
        ...
        cod_alpha = list(self.codon_alignment.values())[0].seq.alphabet
        ...
        self.prot_align = MultipleSeqAlignment(
            list(self.prot_dict.values()), alphabet=generic_protein)
        self.filt_prot_align = MultipleSeqAlignment(
            list(self.filt_prot_align.values()), alphabet=generic_protein)
        self.codon_alignment = codonalign.CodonAlignment(
            list(self.codon_alignment.values()), alphabet=cod_alpha)
        self.fcodon_alignment = codonalign.CodonAlignment(
            list(self.fcodon_alignment.values()), alphabet=cod_alpha)
        # patch tree
        ...
        for aa1, aarea in cur_aa2aa_rea.items():
            ...
            for aa2, species in aarea.items():
                ...
                for genome in self.seqset.common_genome:
                    ...
                    for pos in range(self.global_consensus.allen):
                        pos_best_aa = sorted(
                    ...
                    if self.settings.CODON_COUNT_THRESHOLD > 1:
                        if self.settings.USE_GLOBAL:
                            greacodon = Counter({_cod: _count for _cod, _count in six.iteritems(greacodon) if _count > self.settings.CODON_COUNT_THRESHOLD})
                        else:
                            freacodon = Counter({_cod: _count for _cod, _count in six.iteritems(freacodon) if _count > self.settings.CODON_COUNT_THRESHOLD})
...
def check_align_upgrade(al1, al2, scoring_method, method="wilcoxon", column_list=None):
    ...
    if scoring_method != "identity" and isinstance(scoring_method, six.string_types):
...
def print_data_to_txt(outputfile, header, X, X_label, Y, Y_prob, codon_data, cible, suptext=None, valid={}):
    ...
    if suptext and isinstance(suptext, six.string_types):
        out.write("\n\n{}\n".format(suptext))
    out.close()
